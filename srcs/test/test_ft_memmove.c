/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   test_ft_memmove.c                                  :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: jodufour <jodufour@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/25 23:08:09 by jodufour          #+#    #+#             */
/*   Updated: 2022/04/27 09:43:37 by jodufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <stdio.h>
#include <string.h>
#include "ft_string.h"
#include "tester.h"
#include "e_ret.h"

typedef struct s_basic_test		t_basic_test;
typedef struct s_overlap_test	t_overlap_test;

struct s_basic_test
{
	int const		num;
	uint8_t const	*src;
	uint8_t *const	dst0;
	uint8_t *const	dst1;
	size_t const	size;
	size_t const	n;
};

struct s_overlap_test
{
	int const		num;
	uint8_t *const	area0;
	uint8_t *const	area1;
	size_t const	src_idx;
	size_t const	dst_idx;
	size_t const	size;
	size_t const	n;
};

static t_basic_test const		g_basic_test[] = {
{1,
	(uint8_t[]){0x42, 0x21, 0x00},
	(uint8_t[]){0xff},
	(uint8_t[]){0xff},
	1, 0},
{2,
	(uint8_t[]){0x00, 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77},
	(uint8_t[]){0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa, 0x99, 0x88},
	(uint8_t[]){0xff, 0xee, 0xdd, 0xcc, 0xbb, 0xaa, 0x99, 0x88},
	8, 8},
{3,
	(uint8_t[]){0x40, 0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49,
	0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f},
	(uint8_t[]){0x01, 0x03, 0x05, 0x07, 0x09, 0x11, 0x13, 0x15, 0x17, 0x19,
	0x21, 0x23, 0x25, 0x27, 0x29, 0x31},
	(uint8_t[]){0x01, 0x03, 0x05, 0x07, 0x09, 0x11, 0x13, 0x15, 0x17, 0x19,
	0x21, 0x23, 0x25, 0x27, 0x29, 0x31},
	16, 10},
{4,
	(uint8_t[]){0x00, 0x03, 0x06, 0x09, 0x0c, 0x0f, 0x12, 0x15, 0x18, 0x1b,
	0x1e, 0x21, 0x24, 0x27, 0x2a, 0x2d, 0x30, 0x33, 0x36, 0x39, 0x3c, 0x3f,
	0x42, 0x45, 0x48, 0x4b, 0x4e, 0x51, 0x54, 0x57, 0x5a, 0x5d},
	(uint8_t[]){0xff, 0xfe, 0xfd, 0xfc, 0xfb, 0xfa, 0xf9, 0xf8, 0xf7, 0xf6,
	0xf5, 0xf4, 0xf3, 0xf2, 0xf1, 0xf0, 0xef, 0xee, 0xed, 0xec, 0xeb, 0xea,
	0xe9, 0xe8, 0xe7, 0xe6, 0xe5, 0xe4, 0xe3, 0xe2, 0xe1, 0xe0},
	(uint8_t[]){0xff, 0xfe, 0xfd, 0xfc, 0xfb, 0xfa, 0xf9, 0xf8, 0xf7, 0xf6,
	0xf5, 0xf4, 0xf3, 0xf2, 0xf1, 0xf0, 0xef, 0xee, 0xed, 0xec, 0xeb, 0xea,
	0xe9, 0xe8, 0xe7, 0xe6, 0xe5, 0xe4, 0xe3, 0xe2, 0xe1, 0xe0},
	32, 42},
{5,
	(uint8_t[]){0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
	0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
	0x16, 0x17, 0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x20, 0x21,
	0x22, 0x23, 0x24, 0x25, 0x26, 0x27, 0x28, 0x29, 0x2a, 0x2b, 0x2c, 0x2d,
	0x2e, 0x2f, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39,
	0x3a, 0x3b, 0x3c, 0x3d, 0x3e, 0x3f},
	(uint8_t[]){0x3f, 0x3e, 0x3d, 0x3c, 0x3b, 0x3a, 0x39, 0x38, 0x37, 0x36,
	0x35, 0x34, 0x33, 0x32, 0x31, 0x30, 0x2f, 0x2e, 0x2d, 0x2c, 0x2b, 0x2a,
	0x29, 0x28, 0x27, 0x26, 0x25, 0x24, 0x23, 0x22, 0x21, 0x20, 0x1f, 0x1e,
	0x1d, 0x1c, 0x1b, 0x1a, 0x19, 0x18, 0x17, 0x16, 0x15, 0x14, 0x13, 0x12,
	0x11, 0x10, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08, 0x07, 0x06,
	0x05, 0x04, 0x03, 0x02, 0x01, 0x00},
	(uint8_t[]){0x3f, 0x3e, 0x3d, 0x3c, 0x3b, 0x3a, 0x39, 0x38, 0x37, 0x36,
	0x35, 0x34, 0x33, 0x32, 0x31, 0x30, 0x2f, 0x2e, 0x2d, 0x2c, 0x2b, 0x2a,
	0x29, 0x28, 0x27, 0x26, 0x25, 0x24, 0x23, 0x22, 0x21, 0x20, 0x1f, 0x1e,
	0x1d, 0x1c, 0x1b, 0x1a, 0x19, 0x18, 0x17, 0x16, 0x15, 0x14, 0x13, 0x12,
	0x11, 0x10, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08, 0x07, 0x06,
	0x05, 0x04, 0x03, 0x02, 0x01, 0x00},
	64, 3},
{6,
	(uint8_t[]){},
	(uint8_t[]){},
	(uint8_t[]){},
	128, 18},
{7,
	(uint8_t[]){},
	(uint8_t[]){, 00, 0xff},
	(uint8_t[]){, 00, 0xff},
	256, 100},
{0}
};

static t_overlap_test const		g_overlap_test[] = {
{8,
	(uint8_t[]){0x12, 0x4d, 0x04, 0x5f, 0x01},
	(uint8_t[]){0x12, 0x4d, 0x04, 0x5f, 0x01},
	1, 0, 5, 3},
{9,
	(uint8_t[]){0x3a, 0xe1, 0x55, 0x72, 0x90, 0xbf, 0xc4, 0xf0},
	(uint8_t[]){0x3a, 0xe1, 0x55, 0x72, 0x90, 0xbf, 0xc4, 0xf0},
	0, 2, 8, 6},
{10,
	(uint8_t[]){0x12, 0x43, 0x56, 0x71, 0xf4, 0xde, 0x09, 0x22, 0xa0},
	(uint8_t[]){0x12, 0x43, 0x56, 0x71, 0xf4, 0xde, 0x09, 0x22, 0xa0},
	2, 0, 9, 7},
{11,
	(uint8_t[]){0xfe, 0x12, 0xed, 0x23, 0xdc, 0x34, 0xcb, 0x45, 0xba, 0x56},
	(uint8_t[]){0xfe, 0x12, 0xed, 0x23, 0xdc, 0x34, 0xcb, 0x45, 0xba, 0x56},
	0, 3, 10, 7},
{0}
};

static int	__basic_test(int *const ret)
{
	uint8_t	*addr0;
	uint8_t	*addr1;
	int		i;

	i = 0;
	while (g_basic_test[i].num)
	{
		addr0 = memmove(
				g_basic_test[i].dst0,
				g_basic_test[i].src,
				g_basic_test[i].n);
		addr1 = ft_memmove(
				g_basic_test[i].dst1,
				g_basic_test[i].src,
				g_basic_test[i].n);
		result(g_basic_test[i].num,
			addr0 - g_basic_test[i].dst0
			== addr1 - g_basic_test[i].dst1
			&& !memcmp(g_basic_test[i].dst0, g_basic_test[i].dst1,
				g_basic_test[i].size));
		++i;
	}
	return (*ret = SUCCESS);
}

static int	__overlap_test(int *const ret)
{
	uint8_t	*addr0;
	uint8_t	*addr1;
	int		i;

	i = 0;
	while (g_overlap_test[i].num)
	{
		addr0 = memmove(
				g_overlap_test[i].area0 + g_overlap_test[i].dst_idx,
				g_overlap_test[i].area0 + g_overlap_test[i].src_idx,
				g_overlap_test[i].n);
		addr1 = ft_memmove(
				g_overlap_test[i].area1 + g_overlap_test[i].dst_idx,
				g_overlap_test[i].area1 + g_overlap_test[i].src_idx,
				g_overlap_test[i].n);
		result(g_overlap_test[i].num,
			addr0 - g_overlap_test[i].area0 + g_overlap_test[i].dst_idx
			== addr1 - g_overlap_test[i].area1 + g_overlap_test[i].dst_idx
			&& !memcmp(g_overlap_test[i].area0, g_overlap_test[i].area1,
				g_overlap_test[i].size));
		++i;
	}
	return (*ret = SUCCESS);
}

int	test_ft_memmove(int *const ret)
{
	printf("%20s:", __func__ + 5);
	if (__basic_test(ret) || __overlap_test(ret))
		return (*ret);
	printf("\n");
	return (*ret = SUCCESS);
}
